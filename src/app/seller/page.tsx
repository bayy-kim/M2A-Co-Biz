...
const SIDEBAR: { label: string; href: string; icon: LucideIcon }[] = [
  ...
  { label: "Sales", href: "/seller?tab=sales", icon: ShoppingCart },
  { label: "Payouts", href: "/seller?tab=payouts", icon: Wallet },
]
                    ...
                    {seller.products.slice(0, 5).map((p) => (
                      <tr key={p.id} className="border-b border-outline-variant/20 hover:bg-surface-container-low transition-colors">
                        ...
                        <td className="px-lg py-3">
                          <span className={`inline-flex px-md py-0.5 rounded-full text-label-sm font-bold ${p.status === "ACTIVE" ? "bg-success/10 text-success" : "bg-surface-container-highest text-on-surface-variant"}`}>{p.status}</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
